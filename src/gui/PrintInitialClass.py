...
class PrintInitialClass:
    ...
    def __createUI(self):
        ...
        # 설정값 테스트 버튼 생성
        test_button = tk.Button(left_frame, text="현재 설정값 테스트", command=init_test_callback)
        test_button.pack(pady=2)

        # 새로운 프린터 설정값 입력 엔트리 생성
        new_settings_entry = tk.Entry(left_frame, width=10)
        new_settings_entry.pack(pady=2)

        # 설정값 변경 버튼 생성
        update_button = tk.Button(left_frame, text="설정값 변경")
